...
function sub_crt() {
	...
	if { [[ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ]] || [[ $DIFF == true ]]; } && [[ $SUBCRT == true ]]; then
		...
		if [[ $SUBCRT == false ]]; then
			printf "\n%b[%s] %s skipped due to mode or defined in sleuth.cfg.%b\n" \
				"$yellow" "$(date +'%Y-%m-%d %H:%M:%S')" "${FUNCNAME[0]}" "$reset"
		else
			printf "%b[%s] %s has already been processed. To force execution, delete:\n    %s/.%s%b\n\n" \
				"$yellow" "$(date +'%Y-%m-%d %H:%M:%S')" "${FUNCNAME[0]}" "$called_fn_dir" "/.${FUNCNAME[0]}" "$reset"
...
function virtualhosts() {
	...
	if { [[ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ]] || [[ $DIFF == true ]]; } && [[ $VIRTUALHOSTS == true ]]; then
		...
		if [[ $VIRTUALHOSTS == false ]]; then
			printf "\n${yellow}[$(date +'%Y-%m-%d %H:%M:%S')] ${FUNCNAME[0]} skipped in this mode or defined in sleuth.cfg ${reset}\n"
		else
...
function ipcidr_target() {
	...
	if [[ $1 =~ ^$IP_CIDR_REGEX ]]; then
		echo $1 | mapcidr -silent | anew -q target_sleuth_ipcidr.txt
		if [[ -s "./target_sleuth_ipcidr.txt" ]]; then
			[ "$REVERSE_IP" = true ] && cat ./target_sleuth_ipcidr.txt | hakip2host | cut -d' ' -f 3 | unfurl -u domains 2>/dev/null | sed -e 's/*\.//' -e 's/\.$//' -e '/\./!d' | anew -q ./target_sleuth_ipcidr.txt
			if [[ $(cat ./target_sleuth_ipcidr.txt | wc -l) -eq 1 ]]; then
				domain=$(cat ./target_sleuth_ipcidr.txt)
			elif [[ $(cat ./target_sleuth_ipcidr.txt | wc -l) -gt 1 ]]; then
				unset domain
				list=${PWD}/target_sleuth_ipcidr.txt
			fi
		fi
		if [[ -n $2 ]]; then
			cat $list | anew -q $2
...
function help() {
	...
	printf "   -f config_file    Alternate sleuth.cfg file\n"
...
PROGARGS=$(getopt -o 'd:m:l:x:i:o:f:q:c:z:rspanwvh::' --long 'domain:,list:,recon,subdomains,passive,all,web,osint,zen,deep,help,vps,check-tools' -n 'sleuth' -- "$@")
...
. "${SCRIPTPATH}"/sleuth.cfg || {
	echo "Error importing sleuth.ctg"
	exit 1
}

if [[ -s $CUSTOM_CONFIG ]]; then
	# shellcheck source=/home/six2dez/Tools/sleuth/custom_config.cfg
	. "${CUSTOM_CONFIG}" || {
		echo "Error importing sleuth.ctg"
		exit 1
	}
...
'w')
	if [[ -n $list ]]; then
		...
		printf "\n\n${bred} Web mode needs a website list file as target (./sleuth.sh -l target.txt -w) ${reset}\n\n"
		exit
	fi
	webs_menu
	exit
