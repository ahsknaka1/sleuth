...
CONFIG_FILE="./sleuth.cfg"

if [[ ! -f $CONFIG_FILE ]]; then
	echo -e "${bred}[!] Config file sleuth.cfg not found.${reset}"
	exit 1
fi
...
function banner() {
	...
	cat <<EOF
   ___           _           _     __ _            _   _     
  / _ \_ __ ___ (_) ___  ___| |_  / _\ | ___ _   _| |_| |__  
 / /_)/ '__/ _ \| |/ _ \/ __| __| \ \| |/ _ \ | | | __| '_ \ 
/ ___/| | | (_) | |  __/ (__| |_  _\ \ |  __/ |_| | |_| | | |
\/    |_|  \___// |\___|\___|\__| \__/_|\___|\__,_|\__|_| |_|
              |__/                                           
...
EOF
		...
		echo -e "${byellow}Golang will not be configured according to the user's preferences (install_golang=false in sleuth.cfg).${reset}\n"
	fi
...
function show_additional_help() {
	...
	echo ""
	echo "  ****             Without any arguments, the script will update sleuth"
...
function handle_install_arguments() {
	echo -e "\n${bgreen}sleuth installer/updater script${reset}\n"
	...
	if [[ "$(id -u)" -eq 0 ]]; then
		...
		if ! sudo -n true 2>/dev/null; then
			...
			echo -e "${bred}echo \"${USER}  ALL=(ALL:ALL) NOPASSWD: ALL\" | sudo tee /etc/sudoers.d/sleuth${reset}\n"
		fi
		SUDO="sudo"
	fi
}
